...
class sidebar extends CI_Model
{
    ...
    public function get_sidebars_by_permission($name = "unregistered")
    {
        ...
        $side_bar_ids = $this->get_side_bar_ids_by_permission_id($permission_id);
        ...
        foreach ($side_bar_ids as $side_bar_id) {
            ...
            // returning an array of sub side bar ids
            $sub_side_bar_ids = $this->get_sub_side_bar_ids_by_side_bar_id($side_bar_id);
            foreach ($sub_side_bar_ids as $sub_side_bar_id) {
                //returning sub_side_field info related to the side_bar that we want
                $sub_side_info = $this->get_sub_side_bar_info_by_sub_side_bar_id($sub_side_bar_id);
    ...
    public function get_side_bar_ids_by_permission_id($permission_id = "0")
    {
    ...
    public function get_permission_id_by_permission_name($name = "unregistered"){
        $this->db->select("permission_id", "name");
